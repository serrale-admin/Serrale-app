      ...
      <ScrollView style={{ maxHeight: 420 }} contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
        {AREAS.map((a, i) => {
          ...
          return (
            ...
            >
              <Icon name={i === 0 ? 'ph-buildings' : 'ph-map-pin'} size={18} color={i === 0 ? colors.goldText : colors.success} weight={i === 0 ? 'fill' : 'regular'} />
              <Text style={[styles.label, active && { fontFamily: fonts.bold }]}>{a}</Text>
